...
> *©ᴘᴏᴡᴇʀᴇᴅ ʙʏ Thenux AI*`;
        ...
        conn.ev.on("messages.upsert", async (messageUpdate) => {
            ...
            if (!mek?.message) return; // Safely check if 'mek' and 'mek.message' exist
            
            const messageType = mek.message.conversation || mek.message.extendedTextMessage?.text;
            if (!messageType) return; // If there's no message text, skip further processing

            // Safely check for 'mek.key' before using it
            const messageKey = mek.key ? mek.key.id : null;
            if (!messageKey) return; // Avoid using 'mek.key' if it's undefined
